...
int alloc_rt_sched_group(struct task_group *tg, struct task_group *parent)
{
	...
	////printk("struharv: alloc_rt_sched_group\n");
	...
	for_each_possible_cpu(i) {
		...
		//printk("struharv: alloc_rt_sched_group: cpu(%d)\n", i);
		init_rt_rq(rt_rq);
		rt_rq->rq = cpu_rq(i);

		init_dl_task_timer(dl_se);
		init_struhar_timer(dl_se);
...
static void update_curr_rt(struct rq *rq)
{
	...
	//printk("struharv: update_curr_rt pid: %d \n", curr->pid);
	...
	if (is_dl_group(rt_rq)) {
		//printk("struharv: update_curr_rt > is_dl_group(rt_rq)");
		...
		if (dl_runtime_exceeded(dl_se)) {
			...
			//printk("struharv: update_curr_rt dl_runtime_exceeded \n");
		...
		//printk("struharv: update_curr_rt > NOT is_dl_group(rt_rq) ");	
	}
}
...
{
	//printk("struharv: deadline.yield_task_rt");	
...
int sched_group_set_rt_runtime(struct task_group *tg, long rt_runtime_us)
{
	...
	//printk("struharv: sched_group_set_rt_runtime, rt_runtime_us = %llu\n", rt_runtime_us);
...
int sched_group_set_rt_period(struct task_group *tg, u64 rt_period_us)
{
	...
	//printk("struharv: sched_group_set_rt_period, rt_period_us = %llu\n", rt_period_us);
