...
int dl_init_tg(struct sched_dl_entity *dl_se, u64 rt_runtime, u64 rt_period)
{
	...
	int is_active;
	//printk("struharv: dl_init_tg  rt_runtime = %llu, rt_period = %llu\n", rt_runtime, rt_period);
...
void init_dl_bandwidth(struct dl_bandwidth *dl_b, u64 period, u64 runtime)
{
	...
	//printk("struharv: init_dl_bandwidth  rt_runtime = %llu, rt_period = %llu\n", runtime, period);
...
{
	//printk("struharv: init_dl_bw dl_b->bw = %llu, dl_b->total_bw = %llu\n", dl_b->bw, dl_b->total_bw);
...
static inline void setup_new_dl_entity(struct sched_dl_entity *dl_se)
{
	...
	//printk("struharv: setup_new_dl_entity  \n");
	...
	//printk("struharv: setup_new_dl_entity  dl_se->deadline=%llu,  dl_se->runtime %llu \n", dl_se->deadline, dl_se->runtime);
}
				...
				struct sched_dl_entity *pi_se)
{
	...
	//printk("struharv: deadline.c.replenish_dl_entity\n");
	/*
	...
	while (dl_se->runtime <= 0) {
		...
		dl_se->runtime += pi_se->dl_runtime;
		//printk("struharv: deadline.c.replenish_dl_entity pi_se->dl_period:%llu pi_se->dl_runtime:%llu\n", pi_se->dl_period, pi_se->dl_runtime);
	}
			     ...
			     struct sched_dl_entity *pi_se)
{
	struct dl_rq *dl_rq = dl_rq_of_se(dl_se);
	struct rq *rq = rq_of_dl_rq(dl_rq);
	//printk("struharv: update_dl_entity\n");
	if (dl_time_before(dl_se->deadline, rq_clock(rq)) ||
...
u64 dl_next_period_struhar(struct sched_dl_entity *dl_se)
{
...
void struhar_signal(int signal_num, struct task_struct *task, int value) {
	struct kernel_siginfo info;	
	memset(&info, 0, sizeof(struct kernel_siginfo));
	info.si_signo = signal_num;
	info.si_code = SI_QUEUE;
	info.si_int = value;
	if (send_sig_info(signal_num, &info, task) < 0) {
		//printk("sending error!!!!");
	}
}
...
int start_dl_timer(struct sched_dl_entity *dl_se)
{
	...
	//printk("struharv: deadline.c.start_dl_timer\n");
	/*
	...
	//printk("struharv: deadline.c.start_dl_timer now= %llu act=%llu period=%llu\n", now, act, dl_se->dl_period);
	/*
...
int start_struhar_timer(struct sched_dl_entity *dl_se)
{
	//printk("start_struhar_timer");
	
	struct hrtimer *timer = &dl_se->struhar_timer;
	struct dl_rq *dl_rq = dl_rq_of_se(dl_se);
	struct rq *rq = rq_of_dl_rq(dl_rq);
	ktime_t now, act;
	s64 delta;
	...
	//printk("struharv: deadline.c.start_struhar_timer now= %llu act=%llu period=%llu\n", now, act, dl_se->dl_period);
...
{
	struct hrtimer *timer = &dl_se->dl_timer;
	//printk("struharv: deadline.c.init_dl_task_timer \n");
...
static enum hrtimer_restart struhar_task_timer(struct hrtimer *timer)
{
	//printk("HEY!");
	...
	//printk("struharv: struhar_task_timer\n");
	struhar_signal(44, dl_se->struhar_to_unlock, 124);
...
void init_struhar_timer(struct sched_dl_entity *dl_se)
{
	struct hrtimer *timer = &dl_se->struhar_timer;
	//printk("struharv: deadline.c.init_struhar_timer \n");

	hrtimer_init(timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	timer->function = struhar_task_timer;
}
...
{
	//printk("struharv: dl_check_constrained_dl");	
	struct task_struct *p = dl_task_of(dl_se);
...
static void update_curr_dl(struct rq *rq)
{
	...
	//printk("struharv: deadline.c.update_curr_dl pid: %d\n", curr->pid);	
	...
	//printk("struharv: deadline.c.update_curr_dl dl_se->runtime=%llu \n", dl_se->runtime);

throttle:
	if (dl_runtime_exceeded(dl_se) || dl_se->dl_yielded) {
		dl_se->dl_throttled = 1;
		//printk("struharv: deadline.c.update_curr_dl throttled\n");
...
{
	//printk("struharv: deadline.yield_task_dl");	
	/*
